...
/*
よく考えたら値のついていないところに値をつける方法（次元下げetc.）が成功しているのにスパースにする方法が成功するわけないよね
*/
...
bool read_onegame(istream& is, Game * game)
{
	ASSERT(game != nullptr);
	...
	if (!getline(is, line)) { cout << "read line1 error!" << endl; return false; }
	...
	//一行目読み込み<棋譜番号> <対局開始日> <先手名> <後手名> <勝敗(0:引き分け,1:先手勝ち,2:後手勝ち)> <手数> 
	header >> gameid >> (game->day) >> (game->black_P) >> (game->white_P) >> game->result >> game->ply;
	ASSERT(0 <= game->result&&game->result <= 2);

	//2行目の読み込み（6文字ずつ並んだ指し手）
	if (!getline(is, line)) { cout << "read line2 error!" << endl;  return false; }

	//指し手の情報から合法手であればその指し手を指し手情報に変換
